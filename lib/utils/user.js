...
const intercomDeleteBatch = batch({maxWait: 1000, maxItems: 100})
  .on('data', function(userIds) {
    var reqData = userIds.map((userId) => {
      return {
        'delete': {user_id: userId}
      };
    });

    console.log('Deleting', userIds, 'from intercom');
    intercom.users.bulk(reqData);
  });
  ...
  return getUserTraits(userId).then(function(userTraits) {
    ...
    if (userTraits) {
      ...
      if (userTraits.database_only || userTraits.num_groups === 0) {
        ...
        intercomDeleteBatch.write(String(userId));
